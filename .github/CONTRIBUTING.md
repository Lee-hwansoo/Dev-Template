...
2. **커밋은 Conventional Commits** (`type(scope): subject`, 제목 한 줄). 히스토리가 곧 변경 기록이므로
   ...
   **브랜치는 `<type>/<주제>`** — type 은 위 표에서, 주제는 kebab-case 한두 단어입니다
   (`fix/detector-cache`, `refactor/streamline-architecture`, `ci/image-tiers`). 한 브랜치는
   한 주제이고, 여러 type 이 섞이면 **지배적인 type** 을 씁니다(수정 40 개 + 계약 15 개 = `fix/`).
   `main` 에 직접 커밋하지 않고, 병합 뒤에는 브랜치를 지웁니다 — 태그가 리비전을 기억하므로
   병합된 브랜치는 남길 이유가 없습니다. 일부만 병합됐다면 남은 커밋을 새 `main` 위로 리베이스합니다.
