...
배포된 운영 환경(로봇 본체, 클라우드 서버 등)은 리소스 절약과 보안을 위해 GUI를 실행하지 않는 **Headless 모드**로 동작합니다. 따라서 시각화는 네트워크로 연결된 **내 노트북(클라이언트)의 Dev 환경**을 활용하는 분산 아키텍처를 권장합니다.

### 1. ROS 2 환경 (DDS Discovery 활용)

로봇과 노트북이 같은 Wi-Fi/네트워크에 있을 때, 논리적 채널을 맞춰 자동으로 화면을 공유합니다.

- **로봇 (Prod)**: `.env`에 `ROS_DOMAIN_ID=10` 설정 후 `make ros-prod` 실행 (데이터 송신만 수행)
- **내 노트북 (Dev)**: `.env`에 동일하게 `ROS_DOMAIN_ID=10` 설정 후 `make ros` 실행. 컨테이너 내부에서 `rviz2`를 켜면 로봇의 센서 데이터가 내 노트북 GPU를 통해 부드럽게 렌더링됩니다.

### 2. ROS 1 환경 (Master-Slave 구조)

ROS 1은 Master의 IP를 명시적으로 지정하여 연결해야 합니다. (예: 로봇 IP `192.168.0.100`, 내 노트북 `192.168.0.200`)

- **로봇 (Prod)**: `.env`에 `ROS_MASTER_URI=http://192.168.0.100:11311` 및 `ROS_IP=192.168.0.100` 설정 후 실행.
- **내 노트북 (Dev)**: `.env`의 `ROS_MASTER_URI`를 로봇의 IP(`http://192.168.0.100:11311`)로 맞추고 `make ros` 실행 후 `rviz`를 켭니다.

### 3. 일반 순수 C++/Python 환경 (Dev)

ROS 프레임워크를 사용하지 않는 경우, 아래의 모던한 원격 시각화 방식을 권장합니다.

- **웹 대시보드 (Web Dashboard)**: 배포 서버(Prod)에서 FastAPI, Flask 등을 띄워 데이터나 MJPEG 비디오 스트림을 제공하고, 노트북 브라우저로 접속하여 확인합니다. (가장 권장)
- **소켓 통신 (ZMQ / WebSockets)**: 배포 서버에서 연산된 결과(좌표, 이미지 프레임)를 소켓으로 퍼블리시하고, 노트북(Dev)에서 이를 수신하여 내 화면에 띄웁니다.
