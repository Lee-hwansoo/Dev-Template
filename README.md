...
| **`make verify`** | **무결성 검증**: 문법·Makefile·호스트 감지·APT 태그 필터·셸 환경 동등성·GPG 핀·정리 의미론·SIF 파이프라인·IDE 설정·보안 기본값 등 **42개 계약**을 실행으로 검증 (약 2초) |
...
> **알려진 제약**: 단일 노드 배치 작업만 지원합니다. `srun` 이 요청한 태스크 수만큼 프로세스를
