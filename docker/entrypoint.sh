...
# Persist the resolved ROS/DDS settings for shells that never run init_bash.sh
# (`docker exec <cmd>` with no login and no BASH_ENV), or such a job would use
# a different DDS configuration. Gated on ROS being installed — compose sets
# ROS_DISTRO everywhere, so its presence proves nothing.
