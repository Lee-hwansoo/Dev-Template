...
* **`make verify`**: 계약을 실행으로 검증합니다 — 문자열 존재가 아니라 동작을 확인하므로
  docker 도 `.env` 도 필요 없습니다. 검사마다 슬러그(`[env-bridge]` 등)가 붙어 있어
  실패 메시지가 어느 계약인지 바로 알려주고, 문서는 `check [슬러그]`로 인용합니다.
  전체 목록은 `bash scripts/verify_repo.sh`의 출력 자체이며, 각 검사가 왜 있는지는
  `scripts/verify_repo.sh`의 슬러그 헤더에 한 문단으로 적혀 있습니다.
