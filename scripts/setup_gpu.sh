...
__gpu_persist() {
    ...
    [ -w "$(dirname "$GPU_ENV_FILE")" ] 2>/dev/null || return 0
    {
        ...
        # Reset first: a new shell reads the image default (/etc/profile.d) and
        # then this file, so a mode switch must clear the previous mode's
        # variables here too, not only in the shell that ran it.
        echo "unset ${GPU_ENV_VARS[*]}"
        for var in "${GPU_ENV_VARS[@]}"; do
            [ -n "${!var:-}" ] && printf 'export %s=%q\n' "$var" "${!var}"
        done
        # The mode that was actually applied ('auto' resolved), for `gpu status`.
        printf 'export DEVKIT_GPU_MODE_ACTIVE=%q\n' "$__gpu_req_mode"
...
case "$__gpu_req_mode" in
    ...
    status)
        ...
        __gpu_row "GPU_MODE" "${GPU_MODE:-auto} (requested) → ${DEVKIT_GPU_MODE_ACTIVE:-not applied} (active)"
...
export DEVKIT_GPU_MODE_ACTIVE="$__gpu_req_mode"
